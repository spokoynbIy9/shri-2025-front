...
interface HighlightReportState {
  ...
  setIsSuccessProcessedById: (id: string, isSuccessProcessed: boolean) => void;
...
export const useHighlightReportStore = create<HighlightReportState>(
  (set, get) => ({
    ...
    setIsSuccessProcessedById: (id, isSuccessProcessed) => {
      set((state) => ({
        highlightReports: state.highlightReports.map((report) =>
          report.id === id ? { ...report, isSuccessProcessed } : report
        ),
      }));
    },
    ...
    getDetailedInfoById: (id) => {
      ...
      const curHighlight = highlightReports.find((report) => report.id === id);
    ...
    getCorrectTitlesForDetailedInfo: (curDetailedInfo) => {
      if (curDetailedInfo) {
        return Object.entries(curDetailedInfo).map(([curTitle, value]) => [
    ...
    removeHighlightReportByIdFromLS: (id: string) => {
      const filteredHighlightReports = removeHighlightReportByIdFromStorage(id);
